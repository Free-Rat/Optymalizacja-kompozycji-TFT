...
def main(nchampions , ntraits):
    ...
    newfuncje.write_to_file(newfuncje.F_NAME , 'n_champions', cu_champs , " " , 'n_traits' , cu_traits , " ")
    
    # generating optimal team using randomised greedy algorithm
    t_greedy = threading.Thread(target=newfuncje.generate_optimal_team_g, args=(queue,champion_pool, TEAM_CAPACITY, IMORTANCE_OF_COMBO, weights, active_limits, tabs , N_greedy))
    t_greedy.start()

    # generating optimal team using bruteforse
    t_bruteforce = threading.Thread(target=newfuncje.generate_optimal_team_bf, args=(queue,champion_pool, TEAM_CAPACITY, IMORTANCE_OF_COMBO, weights, active_limits))
    t_bruteforce.start()

    # genreating random team and calculating average value and max value
    #t_random = threading.Thread(target=newfuncje.generate_random_team , args=(queue,champion_pool, TEAM_CAPACITY,IMORTANCE_OF_COMBO,weights,active_limits,N_random))
    #t_random.start()
    #t_random.join()
    #average_v , max_v , time_r  = queue.get()
    average_v , max_v , time_r  = newfuncje.generate_random_team(queue,champion_pool, TEAM_CAPACITY,IMORTANCE_OF_COMBO,weights,active_limits,N_random)
...
# przesunac wypisywanie do funkcji
#zrobic zapis do pliku
# metaheurystyka
# testy poprawnosci
"""
schemat pliku:
